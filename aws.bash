...
if [[ -f /etc/aws.conf ]]; then
	echo -e "${Green_font_prefix} 你已经登录过aws密钥存放于/etc/aws.conf ${Font_color_suffix}"
...
getvps
iipp
echo && stty erase '^H' && read -p "需要解绑的IP名: " ipname
...
function delip(){
iipp
echo && stty erase '^H' && read -p "需要删除的IP名: " ipname
...
iipp
getvps
echo && stty erase '^H' && read -p "需要绑定的IP名: " ipname
...
function openport(){
getvps
echo && stty erase '^H' && read -p "需要开放的起始端口（默认0）: " startport
echo && stty erase '^H' && read -p "需要开放的终止端口（默认65535）: " stopport
echo && stty erase '^H' && read -p "需要开放的端口类型（tcp/udp/all默认all）: " protocol
...
function setsize(){
echo -e "${Green_background_prefix}
(以下是Linux套餐 不可安装windows)
1)1H 512M 20G 1T 5美元/月
2)1H 1024M 30G 2T 10美元/月
3)1H 2048M 40G 3T 20美元/月
4)2H 4096M 60G 4T 40美元/月
5)2H 8192M 80G 5T 80美元/月
(以下是windows套餐 不可安装linux)
6)1H 512M 30G 1T 10美元/月
7)1H 1024M 40G 2T 17美元/月
8)1H 2048M 50G 3T 30美元/月
9)2H 4096M 60G 4T 55美元/月
10)2H 8192M 80G 5T 100美元/月${Font_color_suffix}
"
...
setos
setsize
echo && stty erase '^H' && read -p "默认root密码: " rootpasswd
...
echo && stty erase '^H' && read -p "是否开启全部端口（默认y）[y/n]: " oport
[[ -z ${oport} ]] && oport="y"
if [[ ${oport} = "y" ]];then
...
echo -e " AWS lightsail管理 ${Red_font_prefix}[v1.0]${Font_color_suffix}
  --- PPX | https://ppx.ink/ ----
  ...
  ${Green_font_prefix}5.${Font_color_suffix} 获取静态IP
  ${Green_font_prefix}6.${Font_color_suffix} 释放静态IP
  ${Green_font_prefix}7.${Font_color_suffix} 删除静态IP
  ${Green_font_prefix}8.${Font_color_suffix} 绑定IP
  ...
  ${Green_font_prefix}15.${Font_color_suffix} 列出所有Zone
